...
def fn_tau_acc(df_fore, df_back, weights, tau_obs, tau_max=False, matched_filter=fn_quadrupole_mf, rot_angle=False, quiet=True):
    ...
    if(len(exp_acc) < len(df_fore)):
        if not quiet: print((len(df_fore)-len(lensing_acc)), 'star pairs have too small impact parameter. Selecting only the good pairs.'); sys.stdout.flush()
        df_back = df_back.iloc[good_bil_ind]; weights = weights[good_bil_ind];
    ...
    observed_acc = np.array([df_back['acc_ra_sub'].to_numpy(), df_back['acc_dec_sub'].to_numpy()]).T
...
def fn_varpi(df_fore, df_back, weights, tau_max=False, matched_filter=fn_parallax_mf, quiet=True):
    ...
    if(len(lensing_varpi) < len(df_back)):
        if not quiet: print((len(df_back)-len(lensing_varpi)), 'star pairs have too small impact parameter. Selecting only the good pairs.'); sys.stdout.flush()
